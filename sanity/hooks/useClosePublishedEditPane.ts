...
import { buildPanesWithoutPublishedEdit } from "../lib/structure-pane-navigation";

export function useClosePublishedEditPane() {
  ...
  return useCallback(() => {
    ...
    const nextPanes = buildPanesWithoutPublishedEdit(currentPanes);
    if (!nextPanes) {
      isClosingRef.current = false;
      return;
    }
