...
import { buildPanesWithoutPendingEdit } from "../lib/structure-pane-navigation";

export function useClosePendingEditPane() {
  ...
  return useCallback(() => {
    ...
    const nextPanes = buildPanesWithoutPendingEdit(currentPanes);
    if (!nextPanes) {
      isClosingRef.current = false;
      return;
    }
